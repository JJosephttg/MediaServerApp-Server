    ...
    for ($true) {
        ...
        if ($addedFiles) {
            ...
            if (!$addedFiles.root.file.length) {
                ...
                $imgPath = $mediaIconDir + $addedFiles.root.file.category + '-' + $addedFiles.root.file.name + $addedFiles.root.file.ext + '.bmp'
                $bitmap.Save($imgPath)
            ...
            #Otherwise, if more than one file, go through each and do necessary steps...     
                ...
                for ($i = 0; $i -lt $addedFiles.root.file.Length; $i++) {
                    ...
                    $enc.Save($srcMemoryStream)
                    $bitmap = New-Object System.Drawing.Bitmap($srcMemoryStream)
                    $imgPath = $mediaIconDir + $addedFiles.root.file[$i].category + '-' + $addedFiles.root.file[$i].name + $addedFiles.root.file[$i].ext + '.bmp'
                    $bitmap.Save($imgPath)
...
Export-ModuleMember Get-FileIcons